...
const selectAllEvents = async () => {
    const result = await connection_1.default.query(`SELECT event_id, title, description, date, location, type,
...
const insertEvent = async (eventData) => {
    const { title, description, date, location, type, price, creator_id, image_url, } = eventData;
    const result = await connection_1.default.query(`INSERT INTO events (title,description,date,location,type,price,creator_id,image_url) VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *`, [title, description, date, location, type, price, creator_id, image_url]);
    // Convert numeric strings to numbers
    const event = result.rows[0];
    if (event && typeof event.price === "string") {
        event.price = Number(event.price);
...
const selectUserEvents = async (user_id) => {
    const result = await connection_1.default.query(`SELECT e.event_id, e.title, e.description, e.date, e.location, e.type, e.price::INT AS price, e.image_url, e.created_at, em.joined_at
     FROM event_members em
